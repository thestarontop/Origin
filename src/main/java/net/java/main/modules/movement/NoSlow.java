...
public class NoSlow extends Module {
    public NoSlow(){
        ...
        this.addValues(withHitting,startTime,mode);
    }
    ...
    public void onPacket(PacketEvent event) {
        ...
        if(mode.getValue().equals("Grim")) {
            ...
            if (event.getPacket() instanceof ServerboundInteractPacket && ((!withHitting.getValue()||!shouldnoslow) && mc.player.isUsingItem())){
                event.cancelEvent();
            }
            if (event.getPacket() instanceof ServerboundSwingPacket && ((!withHitting.getValue()||!shouldnoslow )&& mc.player.isUsingItem())){
                event.cancelEvent();
            }
            //
        }
    ...
    public int getSlot() {
        ...
        for (int i = 0; i <= 8; i++) {
            if (!isUsable(mc.player.getInventory().items.get(i))) {
                return i;
            }
        }
    ...
    public static boolean isScaffoldBlock(ItemStack itemStack) {
        ...
        if (itemBlock.getBlock() == Blocks.GLASS)
            return true;
        return itemBlock.getBlock() != Blocks.TNT
                && itemBlock.getBlock() != Blocks.ANVIL
                && itemBlock.getBlock() != Blocks.BEDROCK
                && itemBlock.getBlock() != Blocks.CHEST
                && itemBlock.getBlock() != Blocks.FURNACE
                && itemBlock.getBlock() != Blocks.COBWEB;
