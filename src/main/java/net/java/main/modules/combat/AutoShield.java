...
public class AutoShield extends Module {
    public AutoShield() {
        super("AutoShield","okay", Module.Category.COMBAT);
    }
    @EventTarget
    public void onUpdate(UpdateEvent event) {
        if (mc.level == null || mc.player == null || mc.player.inventoryMenu.getSlot(45).getItem().getItem() == Items.SHIELD) return;

        for (int a = 0;a <= 44; a++){
            if(mc.player.inventoryMenu.getSlot(a).getItem().getItem() == Items.SHIELD){
                ...
                mc.getConnection().send(new ServerboundContainerClickPacket(0,a+100,a,40, ClickType.SWAP,new ItemStack(Items.SHIELD),int2objectmap));
