...
public class HUD extends Module {
    ...
    public static BooleanValue title = new BooleanValue("Title",true);

    public HUD() {
        super("HUD","HUD", Category.CLIENT, GLFW.GLFW_KEY_UNKNOWN);
        setEnable(true);
        addValues(mode,title);
    }
    ...
    public void onRender(Render2DEvent event) {
        ...
        if (mode.getValue() == "Right") {
            ...
            for (Module module : modenable) {
                mc.font.drawShadow(new PoseStack(), module.getName(), screenWidth - mc.font.width(module.getName()), y, RenderUtils.getRainbowOpaque(0.9f));
                y += 10;
            }
        }else {
            ...
            for (String str : enabled) {
                mc.font.drawShadow(new PoseStack(), str, 4, 14 + y, RenderUtils.getRainbowOpaque(1f));
                y += 10;
            }
        }
