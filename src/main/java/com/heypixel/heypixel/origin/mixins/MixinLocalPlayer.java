...
import com.heypixel.heypixel.origin.main.event.events.MoveEvent;
...
public abstract class MixinLocalPlayer extends AbstractClientPlayer {
    ...
    private void sendPosition() {
        ...
        Origin.getInstance().getEventManager().call(event1);
        }
    /**
     * @author starontop
     * @reason bzd
    ...
    public void aiStep() {
        ...
        if (this.isUsingItem() && !this.isPassenger()) {
            ...
            Origin.getInstance().getEventManager().call(slowdownevent);
            ...
            if(!Origin.getInstance().getModuleManager().getModule("noslow").isEnabled()) {
        ...
        boolean flag4 = (float)this.getFoodData().getFoodLevel() > 6.0F || this.getAbilities().mayfly;
            if ((this.onGround || this.isUnderWater()) && !flag1 && !flag2 && this.hasEnoughImpulseToStartSprinting() && !this.isSprinting() && flag4 && (!this.isUsingItem() || Origin.getInstance().getModuleManager().getModule("noslow").isEnabled()) && !this.hasEffect(MobEffects.BLINDNESS)) {
